...
program MOAB_eval_redist
 ...
 subroutine profile_mesh_redist(srcfile, moab, rc)
  ...
  ! integer, parameter :: numNode = 1639680
  integer, parameter :: numNode = 6480
  ...
  distgrid1 = ESMF_DistGridCreate(minIndex=(/minI/), maxIndex=(/maxI/), rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
    rc=ESMF_FAILURE
    return
  endif

print *, localPet, "# distgrid1 minI = ", minI, " maxI = ", maxI

  minI = (petCount - localPet - 1)*nn + 1
  maxI = (petCount - localPet)*nn
  if (localPet == 0) maxI = numNode

  distgrid2 = ESMF_DistGridCreate(minIndex=(/minI/), maxIndex=(/maxI/), rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
    rc=ESMF_FAILURE
    return
  endif

print *, localPet, "# distgrid2 minI = ", minI, " maxI = ", maxI
 ...
 srcMesh=ESMF_MeshCreate(filename=srcfile, fileformat=ESMF_FILEFORMAT_ESMFMESH, &
  ...
  call ESMF_MeshCreate(srcMesh, nodalDistgrid=distgrid2, rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
    rc=ESMF_FAILURE
    return
  endif
