...
program MOAB_eval_create
  ...
  call profile_mesh_create(.false., file, numNode, rc=localrc)
   if (localrc /=ESMF_SUCCESS) then
  ...
  call profile_mesh_create(.true., file, numNode, rc=localrc)
   if (localrc /=ESMF_SUCCESS) then
     write(*,*) "ERROR IN REDIST SUBROUTINE!"
     stop
    endif
 ...
 subroutine profile_mesh_create(moab, file, numNode, rc)
